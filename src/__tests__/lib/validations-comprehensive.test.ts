...
describe('프론트엔드 폼 전송 시뮬레이션', () => {
    ...
    it('정상 출근', () => {
      expect(
        createAttendanceSchema.safeParse({
          ...
          checkInTime: '2026-02-24T09:00',
          checkOutTime: '2026-02-24T18:00',
        }).success
      ).toBe(true)
    })
    it('모든 enum 허용', () => {
  ...
  describe('기타', () => {
    ...
    it('창고구역', () => {
      expect(createWarehouseZoneSchema.safeParse({ warehouseId: 'w1', zoneCode: 'D', zoneName: 'D구역' }).success).toBe(
        true
      )
    })
    it('부서', () => {
    ...
    it('품질기준', () => {
      expect(
        createQualityStandardSchema.safeParse({
          itemId: 'i1',
          standardName: '외관기준',
          category: '외관검사',
        }).success
    ...
    it('세금계산서', () => {
      expect(
        createTaxInvoiceSchema.safeParse({
