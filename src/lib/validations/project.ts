...
export const createProjectSchema = z.object({
  projectCode: z.string().min(1, '프로젝트 코드를 입력하세요').max(30).regex(/^[A-Za-z0-9-]+$/, '코드는 영문, 숫자, 하이픈만 사용 가능합니다'),
  ...
  startDate: z.string().min(1, '시작일을 입력하세요').regex(/^\d{4}-\d{2}-\d{2}/, '올바른 날짜 형식이 아닙니다'),
...
export const createPayrollSchema = z.object({
  ...
  payDate: z.string().min(1, '지급일을 입력하세요').regex(/^\d{4}-\d{2}-\d{2}/, '올바른 날짜 형식이 아닙니다'),
})
