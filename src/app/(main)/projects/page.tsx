...
const TASK_STATUS: Record<string, string> = { WAITING: '대기', IN_PROGRESS: '진행중', COMPLETED: '완료', ON_HOLD: '보류' }
const PRIORITY_MAP: Record<string, string> = { URGENT: '긴급', HIGH: '높음', NORMAL: '보통', LOW: '낮음' }

const columns: ColumnDef<any>[] = [
  { accessorKey: 'projectCode', header: '코드', cell: ({ row }) => <span className="font-mono text-xs">{row.original.projectCode}</span> },
  { accessorKey: 'projectName', header: '프로젝트명' },
  { id: 'department', header: '부서', cell: ({ row }) => row.original.department?.name || '-' },
  { id: 'period', header: '기간', cell: ({ row }) => `${formatDate(row.original.startDate)} ~ ${row.original.endDate ? formatDate(row.original.endDate) : '미정'}` },
  { id: 'progress', header: '진행률', cell: ({ row }) => <div className="flex items-center gap-2 w-32"><Progress value={Number(row.original.progress)} className="h-2" /><span className="text-xs">{Number(row.original.progress)}%</span></div> },
  { id: 'members', header: '인원', cell: ({ row }) => <span className="text-sm">{row.original.members?.length || 0}명</span> },
...
export default function ProjectsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['projects'] }); setOpen(false); toast.success('프로젝트가 생성되었습니다.') },
    onError: (err: Error) => toast.error(err.message),
  })

  const taskMutation = useMutation({
    mutationFn: ({ projectId, body }: { projectId: string; body: any }) => api.post(`/projects/${projectId}/tasks`, body),
    onSuccess: () => { refreshDetail(); setTaskOpen(false); toast.success('작업이 추가되었습니다.') },
    onError: (err: Error) => toast.error(err.message),
  })

  const memberMutation = useMutation({
    mutationFn: ({ projectId, body }: { projectId: string; body: any }) => api.post(`/projects/${projectId}/members`, body),
    onSuccess: () => { refreshDetail(); setMemberOpen(false); toast.success('멤버가 추가되었습니다.') },
    onError: (err: Error) => toast.error(err.message),
  })
  ...
  const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createMutation.mutate({
      projectCode: form.get('projectCode'), projectName: form.get('projectName'),
      managerId: form.get('managerId'), departmentId: form.get('departmentId'),
      startDate: form.get('startDate'), endDate: form.get('endDate') || undefined,
  ...
  const handleAddTask = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    taskMutation.mutate({
      ...
      body: { taskName: form.get('taskName'), assigneeId: form.get('assigneeId') || undefined, startDate: form.get('startDate') || undefined, endDate: form.get('endDate') || undefined, priority: form.get('priority') || 'NORMAL', estimatedHours: form.get('estimatedHours') ? Number(form.get('estimatedHours')) : undefined },
    })
  }
  ...
  const handleAddMember = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const form = new FormData(e.currentTarget)
    memberMutation.mutate({ projectId: selectedProject.id, body: { employeeId: form.get('employeeId'), role: form.get('role') || 'MEMBER' } })
  }
          ...
          {selectedProject && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card><CardHeader className="pb-2"><CardTitle className="text-xs text-muted-foreground">예산</CardTitle></CardHeader><CardContent><span className="text-sm font-bold">{selectedProject.budget ? formatCurrency(Number(selectedProject.budget)) : '-'}</span></CardContent></Card>
              </div>

              <Tabs defaultValue="tasks">
                <TabsList><TabsTrigger value="tasks"><ListTodo className="mr-1 h-4 w-4" /> 작업 ({selectedProject.tasks?.length || 0})</TabsTrigger><TabsTrigger value="members"><Users className="mr-1 h-4 w-4" /> 멤버 ({selectedProject.members?.length || 0})</TabsTrigger></TabsList>
                <TabsContent value="tasks" className="space-y-3">
                  <Button size="sm" variant="outline" onClick={() => setTaskOpen(true)}><Plus className="mr-1 h-3 w-3" /> 작업 추가</Button>
                  {(selectedProject.tasks || []).length === 0 ? <p className="text-sm text-muted-foreground p-4 text-center">등록된 작업이 없습니다.</p> :
                      ...
                      {(selectedProject.tasks || []).map((t: any) => (
                        <div key={t.id} className="flex items-center justify-between p-3 text-sm">
                          <div className="flex-1">
                            ...
                            <span className="text-muted-foreground text-xs ml-2">{PRIORITY_MAP[t.priority] || t.priority}</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1 w-24"><Progress value={Number(t.progress)} className="h-1.5" /><span className="text-xs">{Number(t.progress)}%</span></div>
                            <Badge variant="outline" className="text-xs">{TASK_STATUS[t.status] || t.status}</Badge>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <TabsContent value="members" className="space-y-3">
                  <Button size="sm" variant="outline" onClick={() => setMemberOpen(true)}><Plus className="mr-1 h-3 w-3" /> 멤버 추가</Button>
                  <div className="border rounded-md divide-y">
                    {(selectedProject.members || []).map((m: any) => (
                      <div key={m.id} className="flex items-center justify-between p-3 text-sm">
                        ...
                        <Badge variant="outline">{m.role === 'PM' ? 'PM' : m.role === 'REVIEWER' ? '검토자' : '멤버'}</Badge>
                      </div>
                    ))}
                  </div>
                </TabsContent>
      ...
      <Dialog open={taskOpen} onOpenChange={setTaskOpen}>
        <DialogContent className="max-w-sm sm:max-w-xl md:max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>작업 추가</DialogTitle></DialogHeader>
          <form onSubmit={handleAddTask} className="space-y-4">
            ...
            <div className="space-y-2"><Label>예상시간 (h)</Label><Input name="estimatedHours" type="number" /></div>
            <Button type="submit" className="w-full" disabled={taskMutation.isPending}>작업 추가</Button>
          </form>
        </DialogContent>
      </Dialog>
