...
export default function GeneralBoardPage() {
  ...
  const commentMutation = useMutation({
    ...
    onSuccess: async () => {
      if (selectedPost) {
        const res = await api.get(`/board/posts/${selectedPost.id}`) as any
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-4">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-sm sm:max-w-xl md:max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>게시글 작성</DialogTitle></DialogHeader>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-sm sm:max-w-xl md:max-w-2xl max-h-[85dvh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" onClick={() => newComment.trim() && commentMutation.mutate({ postId: selectedPost.id, content: newComment })} disabled={commentMutation.isPending || !newComment.trim()}>등록</Button>
                </div>
              </div>
            </div>
          )}
