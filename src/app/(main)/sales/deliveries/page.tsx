...
export default function DeliveriesPage() {
  ...
  const columns: ColumnDef<any>[] = [
    { accessorKey: 'deliveryNo', header: '납품번호', cell: ({ row }) => <span className="font-mono text-xs">{row.original.deliveryNo}</span> },
    { id: 'deliveryDate', header: '납품일', cell: ({ row }) => formatDate(row.original.deliveryDate) },
    { id: 'orderNo', header: '발주번호', cell: ({ row }) => <span className="font-mono text-xs">{row.original.salesOrder?.orderNo || '-'}</span> },
    ...
    { id: 'total', header: '합계', cell: ({ row }) => <span className="font-medium">{formatCurrency(row.original.details?.reduce((s: number, d: any) => s + Number(d.amount), 0) || 0)}</span> },
  ...
  const exportColumns: ExportColumn[] = [
    ...
    { header: '합계', accessor: (r) => formatCurrency(r.details?.reduce((s: number, d: any) => s + Number(d.amount), 0) || 0) },
  ...
  const handleExport = (type: 'excel' | 'pdf') => {
    ...
    const cfg = { fileName: `납품목록_${tabLabel}`, title: `납품관리 목록 (${tabLabel})`, columns: exportColumns, data: currentDeliveries }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-md border">
              <table className="w-full text-sm">
                <thead><tr className="border-b bg-muted/50"><th className="p-2 text-left">품목</th><th className="p-2 w-24">수량</th><th className="p-2 w-32">단가</th><th className="p-2 w-32">금액</th><th className="p-2 w-10"></th></tr></thead>
          ...
          <Button type="submit" className="w-full" disabled={createMutation.isPending}>{createMutation.isPending ? '등록 중...' : '납품 등록'}</Button>
        </form>
      </DialogContent>
    </Dialog>
  )
          ...
          {trackingRows.length > 0 && (
            <div className="space-y-2">
              ...
              {trackingRows.length > 5 && (
                <p className="text-sm text-muted-foreground">외 {trackingRows.length - 5}건</p>
              )}
              <Button
                className="w-full"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="ONLINE">
          <div className="space-y-4">
            ...
            <DataTable columns={columns} data={onlineDeliveries} searchColumn="deliveryNo" searchPlaceholder="납품번호로 검색..." isLoading={onlineLoading} pageSize={50} onExport={{ excel: () => handleExport('excel'), pdf: () => handleExport('pdf') }} />
          </div>
        </TabsContent>
