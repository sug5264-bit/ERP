...
export default function LeavePage() {
  ...
  const columns: ColumnDef<LeaveRow>[] = [
    ...
    { header: '직급', cell: ({ row }) => row.original.employee.position?.name || '-' },
    {
      header: '휴가유형',
      cell: ({ row }) => (
        <Badge variant="outline">{LEAVE_TYPE_MAP[row.original.leaveType] || row.original.leaveType}</Badge>
      ),
    },
    {
      header: '기간',
    ...
    { accessorKey: 'days', header: '일수', cell: ({ row }) => `${row.original.days}일` },
    {
      header: '사유',
      cell: ({ row }) => (
        <span className="block max-w-[150px] truncate" title={row.original.reason || '-'}>
          {row.original.reason || '-'}
        </span>
      ),
    },
        ...
        return (
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation()
                handleAction(id, 'reject', employee.nameKo)
              }}
              disabled={actionMutation.isPending}
            >
  ...
  return (
    <div className="space-y-6">
          ...
          <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
            ...
            <form onSubmit={handleCreate} className="space-y-5">
              ...
              <div className="space-y-4 rounded-md border p-4">
                <h3 className="text-muted-foreground text-sm font-semibold">신청정보</h3>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <Label>
                      휴가유형 <span className="text-destructive">*</span>
                    </Label>
                ...
                <div className="space-y-2">
                  <Label>사유</Label>
                  <Textarea name="reason" placeholder="휴가 사유를 입력하세요" rows={3} />
                </div>
              </div>
                ...
                <div className="flex flex-wrap items-center gap-1">
                  {approvalSteps.map((s, idx) => (
                    <div key={`step-vis-${idx}-${s.lineLabel}`} className="flex items-center gap-1">
                      ...
                      {idx < approvalSteps.length - 1 && <ArrowRight className="text-muted-foreground h-3 w-3" />}
                    </div>
                  ))}
                </div>
                {/* 상세 설정 */}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                ...
                <div>
                  ...
                  <span className="font-mono text-xs">{selectedLeave.employee.employeeNo}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">부서:</span> {selectedLeave.employee.department?.name || '-'}
                </div>
                <div>
                  <span className="text-muted-foreground">직급:</span> {selectedLeave.employee.position?.name || '-'}
                </div>
                <div>
                  <span className="text-muted-foreground">휴가유형:</span>{' '}
                  <Badge variant="outline">{LEAVE_TYPE_MAP[selectedLeave.leaveType] || selectedLeave.leaveType}</Badge>
                </div>
                <div>
                  <span className="text-muted-foreground">일수:</span> {selectedLeave.days}일
                </div>
                <div className="col-span-2">
                  <span className="text-muted-foreground">기간:</span> {formatDate(selectedLeave.startDate)} ~{' '}
                  {formatDate(selectedLeave.endDate)}
                </div>
                <div className="col-span-2">
                  <span className="text-muted-foreground">상태:</span>{' '}
                  <Badge variant={STATUS_MAP[selectedLeave.status]?.variant || 'outline'}>
                    {STATUS_MAP[selectedLeave.status]?.label || selectedLeave.status}
                  </Badge>
                </div>
              </div>
              ...
              {selectedLeave.reason && (
                <div className="rounded-md border p-3">
                  ...
                  <p className="mt-1 text-sm whitespace-pre-wrap">{selectedLeave.reason}</p>
                </div>
              )}
                ...
                <div className="flex flex-wrap items-center gap-1">
                  {DEFAULT_APPROVAL_LINE.map((label, idx) => (
                    <div key={`detail-${label}`} className="flex items-center gap-1">
                      ...
                      {idx < DEFAULT_APPROVAL_LINE.length - 1 && (
              ...
              {selectedLeave.status === 'REQUESTED' && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Button
                      className="flex-1"
                      onClick={() => handleAction(selectedLeave.id, 'approve', selectedLeave.employee.nameKo)}
                      disabled={actionMutation.isPending}
                    >
                    ...
                    <Button
                      variant="destructive"
                      className="flex-1"
                      onClick={() => handleAction(selectedLeave.id, 'reject', selectedLeave.employee.nameKo)}
                      disabled={actionMutation.isPending}
                    >
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (batchConfirm) batchMutation.mutate({ ids: batchConfirm.ids, action: batchConfirm.action })
        }}
