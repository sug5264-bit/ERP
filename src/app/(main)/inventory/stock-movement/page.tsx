...
const columns: ColumnDef<MovementRow>[] = [
  ...
  { header: '이동일', cell: ({ row }) => formatDate(row.original.movementDate) },
  {
    header: '유형',
    cell: ({ row }) => {
      const t = MOVEMENT_TYPE_MAP[row.original.movementType]
      return t ? <Badge variant={t.variant}>{t.label}</Badge> : row.original.movementType
    },
  },
...
export default function StockMovementPage() {
  ...
  const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      toast.error('입고 창고를 선택해주세요.')
      return
    }
    if (movementType === 'TRANSFER' && sourceWarehouseId === targetWarehouseId) {
      toast.error('출발 창고와 도착 창고가 동일할 수 없습니다.')
      return
    }
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <div className="space-y-2">
                  <Label>
                    이동일자 <span className="text-destructive">*</span>
                  </Label>
                  <Input name="movementDate" type="date" required aria-required="true" />
                </div>
                <div className="space-y-2">
                  <Label>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {(movementType === 'OUTBOUND' || movementType === 'TRANSFER') && (
                  <div className="space-y-2">
                    <Label>
                      출고창고 <span className="text-destructive">*</span>
                    </Label>
                ...
                {(movementType === 'INBOUND' || movementType === 'TRANSFER' || movementType === 'ADJUSTMENT') && (
                  <div className="space-y-2">
                    <Label>
                      입고창고 <span className="text-destructive">*</span>
                    </Label>
                    ...
                    <div key={`detail-${idx}-${d.itemId}`} className="space-y-2 rounded-md border p-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="bg-muted/50 flex h-9 items-center justify-end rounded-md border px-3 font-mono text-sm">
          ...
          ...columns,
          {
            ...
            cell: ({ row }: any) => (
              <Button
                ...
                onClick={() => handleDelete(row.original.id, row.original.movementNo)}
                aria-label="삭제"
              >
