...
export default function BudgetPage() {
  ...
  const { data: yearsData } = useQuery({
    ...
    queryFn: () => api.get('/accounting/fiscal-years') as Promise<any>,
  })
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/accounting/budget?${qp.toString()}`) as Promise<any>,
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Select value={yearFilter} onValueChange={setYearFilter}>
          ...
          <SelectContent>
            ...
            {years.map((y: any) => <SelectItem key={y.id} value={y.id}>{y.year}년</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border p-8 text-center text-muted-foreground">등록된 예산이 없습니다.</div>
      ) : (
        <div className="space-y-6">
          {budgets.map((budget) => (
            <Card key={budget.id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle>{budget.department.name} - {budget.fiscalYear.year}년</CardTitle>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="p-2 text-left sticky left-0 bg-muted/50">계정과목</th>
                        {Array.from({ length: 12 }, (_, i) => (
                          <th key={i} className="p-2 text-right whitespace-nowrap">{i + 1}월</th>
                        ))}
                    ...
                    <tbody>
                      {budget.details.map((d) => (
                        <tr key={d.id} className="border-b">
                          <td className="p-2 whitespace-nowrap sticky left-0 bg-background">
                            <span className="font-mono text-xs">{d.accountSubject.code}</span> {d.accountSubject.nameKo}
                          </td>
                          {[d.month01, d.month02, d.month03, d.month04, d.month05, d.month06,
                            d.month07, d.month08, d.month09, d.month10, d.month11, d.month12].map((m, i) => (
                      ...
                      <tr className="bg-muted/50 font-medium">
                        <td className="p-2 sticky left-0 bg-muted/50">합계</td>
                        {Array.from({ length: 12 }, (_, i) => {
                          const monthKey = `month${String(i + 1).padStart(2, '0')}` as keyof typeof budget.details[0]
                          const total = budget.details.reduce((s, d) => s + Number((d as any)[monthKey] || 0), 0)
                          return <td key={i} className="p-2 text-right font-mono text-xs">{total > 0 ? formatCurrency(total) : '-'}</td>
                        })}
