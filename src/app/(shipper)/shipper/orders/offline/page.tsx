...
function OrdersTab({ posts, onRefresh }: { posts: PostItem[]; onRefresh: () => void }) {
  ...
  const filtered = posts.filter((n) => {
    ...
    if (searchKeyword) return n.content.toLowerCase().includes(searchKeyword.toLowerCase())
    return true
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const content = `[오프라인]${titlePart}${postContent.trim()}`
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Dialog open={writeOpen} onOpenChange={setWriteOpen}>
          ...
          <DialogContent className="max-h-[90vh] max-w-sm overflow-y-auto sm:max-w-lg">
            <DialogHeader>
              <DialogTitle>오프라인 발주 게시글 작성</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  placeholder="오프라인 발주 내용을 입력하세요 (거래처, 품목, 수량 등)..."
                  value={postContent}
      ...
      <div className="space-y-2">
        {filtered.length === 0 && (
          <div className="text-muted-foreground flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
            ...
            <p className="text-sm">등록된 오프라인 발주 게시글이 없습니다.</p>
          </div>
        )}
        {filtered.map((note, idx) => {
          const postNo = filtered.length - idx
          const isExpanded = expandedId === note.id
          const afterChannel = note.content.replace(/^\[오프라인\]/, '')
...
// ── 출고관리 탭 ──
function DeliveriesTab({ posts, statusFilter }: { posts: PostItem[]; statusFilter: string | null }) {
  const [searchKeyword, setSearchKeyword] = useState('')
  const [startDate, setStartDate] = useState('')
  const [endDate, setEndDate] = useState('')
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.map((post, idx) => {
        ...
        const afterChannel = displayContent.replace(/^\[오프라인\]/, '')
        const titleMatch = afterChannel.match(/^\[(.+?)\]\n?/)
        const title = titleMatch ? titleMatch[1] : null
        const body = titleMatch ? afterChannel.slice(titleMatch[0].length) : afterChannel.replace(/^\n/, '')
...
export default function ShipperOfflineOrdersPage() {
  ...
  // Filter offline only
  const offlinePosts = useMemo(() => posts.filter((p) => p.content.startsWith('[오프라인]')), [posts])

  const stats = useMemo(() => {
    const withDp = offlinePosts.filter((p) => p.deliveryPost)
    ...
    return { total: offlinePosts.length, preparing, shipped, delivered }
  }, [offlinePosts])
  ...
  return (
    <ShipperLayoutShell>
      <div className="space-y-6">
        <PageHeader
          title="오프라인 주문 (발주/출고)"
          description="오프라인 발주 게시글을 작성하고 출고 현황을 확인합니다"
        />
        ...
        >
          ...
          <TabsContent value="orders" className="mt-4">
            <OrdersTab posts={offlinePosts} onRefresh={refetch} />
          </TabsContent>
          <TabsContent value="deliveries" className="mt-4">
            <DeliveriesTab posts={offlinePosts} statusFilter={statusFilter} />
          </TabsContent>
        </Tabs>
      </div>
    </ShipperLayoutShell>
