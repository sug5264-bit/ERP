...
export default function ShipperDashboardPage() {
  ...
  const recentOrders = (dd?.recentOrders || []) as Array<{
    id: string; orderNo: string; recipientName: string; itemName: string;
    status: string; createdAt: string; trackingNo?: string
  }>

  const summaryItems = [
    { label: '금일 접수', value: stats?.todayCount || 0, icon: PackagePlus, color: 'text-blue-600', bgColor: 'bg-blue-50' },
    { label: '처리중', value: stats?.processingCount || 0, icon: Clock, color: 'text-yellow-600', bgColor: 'bg-yellow-50' },
    { label: '배송중', value: stats?.inTransitCount || 0, icon: Truck, color: 'text-indigo-600', bgColor: 'bg-indigo-50' },
    { label: '배송완료', value: stats?.deliveredCount || 0, icon: Package, color: 'text-green-600', bgColor: 'bg-green-50' },
  ]

  return (
    <ShipperLayoutShell>
      <div className="space-y-4 sm:space-y-6">
        <PageHeader title="대시보드" description="배송 현황을 한눈에 확인하세요">
            ...
            {recentOrders.length === 0 ? (
              ...
              <div className="space-y-0">
                {recentOrders.slice(0, 8).map((order, idx) => (
                  <div key={order.id} className={`flex items-center justify-between py-2.5 text-sm ${idx < Math.min(recentOrders.length, 8) - 1 ? 'border-b' : ''}`}>
